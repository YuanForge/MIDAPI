...
- [升级与重新部署](#升级与重新部署)
  ...
  - [场景三：修改了前端代码](#场景三修改了前端代码react-页面样式api-路径等)
- [常用维护命令](#常用维护命令)
- [日志说明与排查指南](#日志说明与排查指南)
...
将生成的强随机字符串填入 `server.jwt_secret`，并编辑 `config.yaml` 填写本地服务地址（各字段含义见[配置说明](#配置说明)）：
...
# 上传前端静态资源
scp -r web/app/dist user@your-server:/opt/fanapi/web/
```
...
### 场景三：修改了前端代码（React 页面、样式、API 路径等）
...
npm run build   # 产物输出到 web/app/dist/
cd ../..
```
...
```bash
# 上传新的前端静态资源
scp -r web/app/dist user@your-server:/opt/fanapi/web/dist
...
**原因**：nginx 将 `/admin` 代理到 Go 后端，但后端没有对应的页面路由，返回 404。`/admin` 是 React Router 的前端路由，应回落到 `index.html`。
